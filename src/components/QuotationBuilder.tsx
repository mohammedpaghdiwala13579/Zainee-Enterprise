...
export default function QuotationBuilder() {
  ...
  // Business Branding & Entity state ("zainee")
  ...
  const currentCompany = COMPANY_PROFILES[activeCompany] || COMPANY_PROFILES.zainee;
  const isZainee = true;
  ...
  const handleSwitchCompany = (newCompany: CompanyId) => {
    ...
    try {
      const leavingDraft = {
        ...
        vatPercent,
        transportationFee,
        currentDocId: (currentDocId && currentDocId.startsWith("ze-")) ? currentDocId : null,
        rows,
        mergedRegions,
        cellFormats
      };
    ...
    if (targetDraft) {
      ...
      const validDocId = targetDraft.currentDocId && targetDraft.currentDocId.startsWith("ze-") ? targetDraft.currentDocId : null;
    ...
    showToast(`Active business entity: ${COMPANY_PROFILES.zainee.name}`, "info");
  };
  ...
  const generateUUID = (_company: CompanyId = "zainee") => {
        ...
        >
          ...
          <div className="watermark-container absolute inset-0 pointer-events-none flex items-center justify-center overflow-hidden z-0 select-none">
            <img 
              ...
              className="object-contain select-none max-w-[500px] w-[45%] opacity-[0.035]"
